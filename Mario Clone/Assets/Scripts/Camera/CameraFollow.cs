  ...
  void Awake()
    ...
    void FixedUpdate()
    {
        if (followsPlayer)
        {
            ...
            {
                Vector3 newCamaeraPosition = Vector3.SmoothDamp(transform.position, aheadTargetPos, ref currentVelocity, cameraSpeed);

                transform.position = new Vector3(newCamaeraPosition.x, transform.position.y, newCamaeraPosition.z);
